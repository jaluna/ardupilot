...
// #define NMEA_LOG_PATH "north.log"
...
bool AP_GPS_NORTH::_decode(char c)
{
...
// internal utilities
//
int16_t AP_GPS_NORTH::_from_hex(char a)
{
    if (a >= 'A' && a <= 'F')
        return a - 'A' + 10;
    else if (a >= 'a' && a <= 'f')
...
int32_t AP_GPS_NORTH::_parse_decimal_100(const char *p)
{
    char *endptr = nullptr;
 ...
 */
uint32_t AP_GPS_NORTH::_parse_degrees()
{
    char *p, *q;
    uint8_t deg = 0, min = 0;
    float frac_min = 0;
 ...
 */
bool AP_GPS_NORTH::_have_new_message()
{
    if (_last_RMC_ms == 0 ||
        _last_GGA_ms == 0) {
        return false;
...
bool AP_GPS_NORTH::_term_complete()
{
 ...
 */
bool
AP_GPS_NORTH::_detect(struct NORTH_detect_state &state, uint8_t data)
{
	switch (state.step) {
	case 0:
		state.ck = 0;
